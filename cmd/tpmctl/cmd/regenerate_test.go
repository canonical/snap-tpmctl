...
func TestRegenerateKey(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"Success_on_regenerating_recovery_key": {recoveryKeyName: "test"},

		"Error_on_empty_name":                {wantErr: true},
		"Error_on_regenerating_recovery_key": {recoveryKeyName: "test", wantErr: true},
	}
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			golden.CheckOrUpdate(t, out.String()) // TestRegenerateKey returns the expected output
		})
	}
}
